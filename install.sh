...
cat <<'EOF'
  Get started — open your IDE (Claude Code / Cursor) in this repo and run:
    ...
    /maestro my-feature              # runs .maestro/workflows/sdlc-main.yaml, resumable
                                     # (scaffolds .maestro/runs/my-feature/requirement/
                                     #  on first run and asks you to fill it)

  Design workflows visually: ./maestro ui   (serves the builder, auto-loads your runs;
                             --port to change the port) — or open .maestro/ui/builder.html directly.
  Re-install / add an IDE:   ./maestro install [claude-code|cursor]
  Lint one by hand:          python3 .maestro/engine/maestroctl.py validate <file>
  ...
  Version control — recommended .gitignore for a consumer repo:
    ...
    .maestro/engine/  .maestro/ui/  maestro  install.sh   # regenerated by re-running install.sh
  COMMIT these (they are your feature's source of truth and resume ledger):
    .maestro/workflows/   .maestro/docs/   .maestro/memory/   .maestro/runs/<slug>/
  ...
  Uninstall: delete the regenerated dirs above; .maestro/runs/<slug>/ holds your work — keep it.
EOF
